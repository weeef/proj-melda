...
public:
  ...
  void update() 
  {
    //int i = 0;
     //while(i < 10) {
     if(/*_event_id_is("fake_id")*/_event_id_is("some_id")||_event_id_is("attack_id")||_event_id_is("block_id")||_event_id_is("ability_id")){
     
 
    _add_yaml("battlescreen.yaml", {{"top_index",200}, {"input_index", 350}});
    _add_yaml("threeslime.yaml", {{"top_index", 50}, {"input_index", 350}});
    _add_yaml("threemelda.yaml", {{"top_index", 50}, {"input_index", 350}});
   
  if(_event_id_is("attack_id")){   //attack phase 1
  _add_yaml("battlescreen.yaml", {{"top_index",200}, {"input_index", 350}});
    _add_yaml("twoslime.yaml", {{"top_index", 50}, {"input_index", 350}});
    _add_yaml("threemelda.yaml", {{"top_index", 50}, {"input_index", 350}});
    _add_yaml("ability.yaml",{{"top_index", 50}, {"input_index",350}});
  }else if(_event_id_is("block_id")){
     _add_yaml("battlescreen.yaml", {{"top_index",200}, {"input_index", 350}});
    _add_yaml("threeslime.yaml", {{"top_index", 50}, {"input_index", 350}});
    _add_yaml("threemelda.yaml", {{"top_index", 50}, {"input_index", 350}});
  }
if(_event_id_is("ability_id")){
   _add_yaml("battlescreen.yaml", {{"top_index",200}, {"input_index", 350}});
    _add_yaml("zeroslime.yaml", {{"top_index", 50}, {"input_index", 350}});
    _add_yaml("threemelda.yaml", {{"top_index", 50}, {"input_index", 350}});
    _add_yaml("gameover.yaml", {{"top_index", 50}, {"input_index", 350}});
...
void display(const State &state) {
  ...
  //_add_yaml("dot.yaml", {{"top_index", 200}, {"left_index", 300}});
