...
public:
  ...
  State(istream &is) { is >> point; is >> health; width= 15; height = ; }
  ...
  void update() { if (_received_event() && 'a' == _global_mem[100])  height++, width++;}
};
...
void display(const State &state) {
  ...
  _add_yaml("dot.yaml", {{"top", state.get_width()}, {"left", state.get_height()}});
